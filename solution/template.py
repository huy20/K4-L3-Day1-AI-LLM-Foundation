...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        # Cắt history còn 3 lượt cuối (6 message)
        if len(history) > 6:
            history = history[-6:]
...
def run_assistant(
    ...
    while True:
        ...
        if len(history) > 6:
            history = history[-6:]
        costs = estimate_cost(user_msg, reply)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    header = f"{'Prompt':<40} | {'GPT-4o Response':<40} | {'Mini Response':<40} | {'GPT-4o Latency':<18} | {'Mini Latency':<16}"
    print(header)
    print("-" * 170)
    for result in results:
        ...
        print(f"{prompt:<40} | {gpt4o_resp:<40} | {mini_resp:<40} | {result['gpt4o_latency']:<18} | {result['mini_latency']:<16}")
...
if __name__ == "__main__":
    ...
    #     if key != "history":
    #         print(f"{key}: {value}")
    print(chat_with_system_prompt(system_prompt="Bạn là giáo viên tiểu học, giải thích thật đơn giản cho trẻ 8 tuổi.", user_prompt="Giải thích blockchain là gì?"))
